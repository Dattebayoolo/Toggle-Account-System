...
const app = express();
const client = getSsoClient('toggle-calendar');

app.use(express.json());

app.get('/health', (_req, res) => {
  res.status(200).json({ status: 'ok', service: 'toggle-calendar' });
});
  ...
  try {
    ...
    if (!code || !returnedState || returnedState !== expectedState) {
      return res.status(400).type('html').send(renderPage({
        title: 'Toggle Calendar Sign-in Error',
        eyebrow: 'Toggle Calendar',
  ...
  (req, res) => {
    const body = req.user
      ? `<div class="panel">
         ...
         <div class="actions">
           <a class="button" href="/events">Open calendar</a>
...
app.get(
  ...
  (req, res) => {
    res.type('html').send(renderPage({
      title: 'Toggle Calendar Workspace',
      eyebrow: 'Protected',
      ...
      body: `<div class="panel">
             ...
             <div class="actions">
               <a class="button" href="/api/events">View JSON API response</a>
               <a class="button secondary" href="/logout">Sign out</a>
             </div>`
    }));
  }
...
app.listen(config.calendarPort, () => {
  console.log(`Toggle Calendar listening on port ${config.calendarPort}`);
});
